...
/**
 * JFunctionSlider uses two functions: Integer->Integer and Integer->Float.
 * The first one is used to change model values. For example to make slider
 * to snap on ticks (where minor tick interval = 5 and major tick interval =
 * 10) we may use function (t)->(t / 5) * 5.
 * 
 * The second one is used to add support for float values. For example for
 * range from 0.0 to 1.0 use range from 0 to 100 and set following float
 * function (t)-> t / 100f.
 * 
 * @author andro
 *
