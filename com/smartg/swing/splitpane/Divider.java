			...
			public void mouseEntered(MouseEvent e) {
				if (!dragging) {
					dividerComponent.setCursor(Cursor
							.getPredefinedCursor(Divider.this.node.getOrientation() == SplitConstants.HORIZONTAL_SPLIT
	...
	static class DividerDragHandler extends DragHandler {
		...
		private Bounds checkNodeBounds(final Node node) {
			...
			if (orientation == SplitConstants.HORIZONTAL_SPLIT) {
				int x = p0.x;
				bounds = new Bounds(x, x + node.getDividerWidth(), node.rectangle.x,
						node.rectangle.x + node.rectangle.width);
			} else {
				int y = p0.y;
				bounds = new Bounds(y, y + node.getDividerWidth(), node.rectangle.y,
